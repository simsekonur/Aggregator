...
int main(){
  ...
  try{
    ...
    for (int i=0 ; i < NUMOFRECORDS;i++){
      ...
      generateRandomNumbers(SIZEOFVECTORS,15,numberSequence);
      //normalizeTheVector(numberSequence);
      SQLString tmp = "INSERT INTO leafNodes(temperature) VALUES ('[" ;
      for (int j=0; j < SIZEOFVECTORS;j++){      
        if (j== SIZEOFVECTORS -1){
          tmp += to_string(numberSequence[j]);
        }
        else {
          tmp += to_string(numberSequence[j]);
          tmp += ",";
