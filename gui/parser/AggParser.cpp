...
void AggParser::filterArgs(std::vector<double>& args, const Condition& cond) {
    ...
    args.erase(std::remove_if(args.begin(), args.end(), filter), args.end());
}
...
double AggParser::getNumberVariable(const std::string& name) {
...
void AggParser::parse(const std::string& expString) {
    try {
        ...
        std::cerr << "Error while parsing expression: " << expString << std::endl;
